...
parser.add_argument('--learning-rate',type=float, default=.005, help='Learning rate')
...
def run(args):
    ...
    option_critic_prime = option_critic

    optim = torch.optim.RMSprop(option_critic.parameters(), lr=args.learning_rate)
    pi_scheduler = torch.optim.lr_scheduler.MultiplicativeLR(optim, lambda epi: 0.999)
    ...
    while steps < args.max_steps_total:
        ...
        if episode % 50 == 0:
            if (critic_loss == None):
                print(f"[{episode} (0)]\tpi_loss: {actor_loss.item():.4f}\\t\tSteps:{0}\t\tAvg: {avg_rewards}\t\Switches: {avg_switches}")
            else:
                print(f"[{episode} (0)]\tpi_loss: {actor_loss.item():.4f}\t\tv_loss: {critic_loss.item():.4f}\t\tSteps:{0}\t\tAvg: {avg_rewards}\t\Switches: {avg_switches}")

        # Checks if solved
        if avg_rewards >= 195:
            print("Average rewards is = ", avg_rewards)
            print("Average switches is = ", avg_switches)
            print("Done Training")
            return episode
