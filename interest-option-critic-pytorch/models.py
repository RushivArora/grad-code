...
class PolicyOverOptions(nn.Module):
    def __init__(self, rng, num_states, num_options, eps_start=1.0, eps_min=0.01, eps_decay=int(1e5)):
        ...
        self.linear = nn.Linear(self.num_states, self.num_options, bias=False)
        self.tanh = nn.Tanh()
        self.soft = nn.Softmax(dim=0)

    def Q_omega(self, state, option=None):
        #out = self.tanh(self.linear(state))
        out = self.linear(state)
        return self.soft(out)

    def sample_option(self, state):
        epsilon = self.epsilon
        #probs = self.tanh(self.linear(state))
...
class FixedPolicyOverOptions(nn.Module):
    def __init__(self, rng, num_states, num_options, eps_start=1.0, eps_min=0.01, eps_decay=int(1e5)):
        super(FixedPolicyOverOptions, self).__init__()
        self.rng = rng
        self.num_states = num_states
        self.num_options = num_options
    ...
    def inductive_bias(self):
        room1 = list(range(5)) + list(range(10, 15)) + list(range(20, 26)) + list(range(31, 36)) + list(
            range(41, 46)) + [51]
        room2 = list(range(5, 10)) + list(range(15, 20)) + list(range(26, 31)) + list(range(36, 41)) + list(
            range(46, 51)) + list(range(52, 57))
        room3 = list(range(68, 73)) + list(range(78, 83)) + list(range(89, 94)) + list(range(99, 104)) + [62]
        room4 = list(range(57, 62)) + list(range(63, 68)) + list(range(73, 78)) + list(range(83, 89)) + list(
            range(94, 99))

        with torch.no_grad():
            for i in room1:
                self.linear.weight[0, i] = 5
            for i in room2:
                self.linear.weight[0, i] = 4
            for i in room3 + room4:
                self.linear.weight[0, i] = 0

            for i in room2:
                self.linear.weight[1, i] = 5
            for i in room3:
                self.linear.weight[1, i] = 4
            for i in room1 + room4:
                self.linear.weight[1, i] = 0

            for i in room1 + room2:
                self.linear.weight[2, i] = 0
            for i in room3:
                self.linear.weight[2, i] = 5
            for i in room4:
                self.linear.weight[2, i] = 4

            for i in room4:
                self.linear.weight[3, i] = 5
            for i in room1:
                self.linear.weight[3, i] = 4
            for i in room2 + room3:
                self.linear.weight[3, i] = 0
...
class InterestPolicyOverOptions(nn.Module):
    def __init__(self, rng, num_states, num_options, policy_over_options, k = 0., eps_start=1.0, eps_min=0.01,
        ...
        self.k = k
        self.inductive_bias() # Only if using the fourrooms environment
    ...
    def sample_option(self, obs):
        ...
        #int_func = self.tanh(self.linear(obs))
